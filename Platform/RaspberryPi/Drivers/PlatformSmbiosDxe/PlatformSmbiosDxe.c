  ...
  )
{
  ...
  mMemArrMapInfoType19.EndingAddress = 256;
  Status = mFwProtocol->GetModelInstalledMB (&MB);
  if (Status != EFI_SUCCESS) {
    DEBUG ((DEBUG_WARN, "Couldn't get the board memory size - defaulting to 256 MB: %r\n", Status));
  } else {
    mMemArrMapInfoType19.EndingAddress = MB;
  }

  mMemArrMapInfoType19.EndingAddress *= 1024;
