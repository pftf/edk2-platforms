    ...
    )
{
    ...
    if (Retry == 0) {
        DEBUG ((DEBUG_ERROR, "GenericPhyGetLinkStatus: Auto-negotiation timeout (BMSR=0x%04X)\n", Bmsr));
        return EFI_TIMEOUT;
    }
    ...
    )
{
    ...
    if (Phy->LinkUp != LinkUp) {
        if (LinkUp) {
            DEBUG ((DEBUG_INFO, "GenericPhyUpdateConfig: Link is up\n"));
            ...
            DEBUG ((DEBUG_INFO, "GenericPhyUpdateConfig: Link is down\n"));
        }
    }
